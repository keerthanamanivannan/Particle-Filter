...
def beamRangeFinderModel(zt, xt, m, n, resolution, L):
    ...
    for k in range(len(zt_map)):
        ...
        p = get_pHit(zt_map[k], zt_true[k])# + get_pShort(zt_map[k], zt_true[k]) + get_pMax(zt_map[k]) + get_pRand(zt_map[k])
        print "p: "+ str(p)
        q = p + q
    #print "here"
    return q
...
def main():
    m, global_mapsize_x, global_mapsize_y, resolution, autoshifted_x, autoshifted_y = mapParser.parser()
