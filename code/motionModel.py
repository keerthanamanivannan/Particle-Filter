...
def motionModel(uCurrent, uPrev, xPrev, alpha):
	...
	trueRot1 = odomRot1 - sample(0, alpha[0]*odomRot1 + alpha[1]*odomTrans)

	#print "trans: " + str(alpha[2]*odomTrans + alpha[3]*(odomRot1 + odomRot2))
	trueTrans = odomTrans -  sample(0,alpha[2]*odomTrans + alpha[3]*(odomRot1 + odomRot2))

	#print "rot2: " + str(alpha[0]*odomRot2 + alpha[1]*odomTrans)
	trueRot2 = odomRot2 - sample(0, alpha[0]*odomRot2 + alpha[1]*odomTrans)
...
def sample(mu, sigma):
	return np.random.normal(mu, math.fabs(sigma))
